...
// }
  ...
  ngOnInit(): void {
    ...
    if (currency_id) {
      this.fetchVendortData(currency_id);
    }
...
fetchVendortData(vendor_id:string){

  this._VendorService.showVendorById(vendor_id).subscribe({
    next: (response) => {
      if (response) {
  ...
  handleForm() {
   ...
   this.addresses.controls.forEach((element,index) => {
      ...
      if(client_id){
      this._VendorService.updateVendor(client_id,formData).subscribe({
        next: (response) => {
          console.log(response);
          if (response) {
