...
@Component({
  ...
  styleUrl: './update-purchase-order.component.css'
})
export class UpdatePurchaseOrderComponent {
  ...
  ngOnInit(): void {
    ...
    if (returnSaleId) {
      this.loadPurchaseOrder(returnSaleId);
    }
  }
    ...
    this._PurchaseOrdersService.getPurchaseOrderById(id).subscribe({
      next: (response) => {
        if (response) {
          ...
          this.selectedVendor = orderData.vendor;
          this.selectedStore = orderData.store;
         ...
         while (this.items.length !== 0) {
          ...
          orderData.items.forEach((item: any) => {
            ...
            itemGroup.patchValue({
              ...
              determinants: item.product_branch?.determinants
              },
              type: 'branch',
  ...
  handleForm() {
    ...
    if (this.purchasesBillForm.valid) {
      ...
      if (!error) {
        ...
        this._PurchaseOrdersService.updatePurchaseOrder(formData,returnSaleId).subscribe({
          next: (response) => {
            if (response) {
